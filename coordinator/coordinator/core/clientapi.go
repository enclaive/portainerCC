...
func (c *Core) UpdateManifest(ctx context.Context, rawUpdateManifest []byte, updater *user.User) error {
	...
	// add new secrets
	for secretName, secret := range updateManifest.Secrets {
		c.zaplogger.Info(secretName + ": " + secret.Type)
		if secret.UserDefined {
			_, err := txdata.getSecret(secretName)
			if err != nil {
				if err := txdata.putSecret(secretName, secret); err != nil {
					return err
				}
			}
			// add Permissions to User portainer
			portainerUser, err := txdata.getUser("portainer")
			portainerUser.Assign(user.NewPermission("readsecret", []string{secretName}))
			portainerUser.Assign(user.NewPermission("writesecret", []string{secretName}))
			for id, perm := range portainerUser.Permissions() {
				c.zaplogger.Info(id + ": ")
				for id := range perm.ResourceID {
					c.zaplogger.Info(id)
				}
			}
			txdata.putUser(portainerUser)
		} else {
			c.zaplogger.Error("new Secrets must be UserDefined")
			return err
		}
...
func (c *Core) GetSecrets(ctx context.Context, requestedSecrets []string, client *user.User) (map[string]manifest.Secret, error) {
	...
	for id, perm := range client.Permissions() {
		c.zaplogger.Info(id + ": ")
		for id := range perm.ResourceID {
			c.zaplogger.Info(id)
		}
...
func (c *Core) WriteSecrets(ctx context.Context, rawSecretManifest []byte, updater *user.User) error {
	...
	for secretName, secret := range newSecrets {
		...
		c.zaplogger.Info("secret set", zap.String("user", updater.Name()), zap.String("secret", secretName), zap.String("type", secret.Type))
	}
	if err := txdata.appendUpdateLog(c.updateLogger.String()); err != nil {
		return err
	}
